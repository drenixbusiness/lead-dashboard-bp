...
const FIXED_MAX        = 15;  // Our internal max target
const FIXED_MARKET_MAX = 7;   // Market avg max (external benchmark)
const FIXED_NORMAL     = 6;   // Our standard
const FIXED_MIN        = 4;   // Minimum acceptable

const LEGEND_ITEMS = [
  { color: '#085041', label: 'Max (15%)',            dash: [8, 3] },
  { color: '#F59E0B', label: 'Market avg max (7%)',  dash: [8, 3] },
  { color: '#1D9E75', label: 'Normal (6%)',          dash: [6, 3] },
  { color: '#E24B4A', label: 'Min (4%)',             dash: [6, 3] },
  { color: '#534AB7', label: 'All-sources rate',     dash: null, thick: true, dot: true },
];

function computeStats(data: LeadsDataRow[]) {
  if (data.length === 0) return { avgRate: 0, aboveMarket: 0, aboveNormal: 0, belowMin: 0 };
  const rates = data.map((r) => r.hire_rate_pct);
  const avgRate = Math.round((rates.reduce((s, v) => s + v, 0) / rates.length) * 10) / 10;
  return {
    avgRate,
    aboveMarket: rates.filter((v) => v >= FIXED_MARKET_MAX).length,
    aboveNormal: rates.filter((v) => v >= FIXED_NORMAL && v < FIXED_MARKET_MAX).length,
    belowMin:    rates.filter((v) => v < FIXED_MIN).length,
  };
}
...
export default function HireRateBandsChart({ data }: { data: LeadsDataRow[] }) {
      ...
      tooltip: {
        callbacks: {
          label: (ctx: TooltipItem<'line'>) => {
            ...
            if (ctx.dataset.label === 'All-sources rate') return ` Overall hire rate: ${Number(val).toFixed(1)}%`;
            ...
            if (ctx.dataset.label === 'Min')             return ` Min threshold: ${FIXED_MIN}%`;
  ...
  const pills = [
    { label: 'Avg overall hire rate',  value: `${stats.avgRate}%` },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ marginBottom: 6 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: '#1a1a1a' }}>
          Performance bands — hire rate (all sources)
        </div>
        <div style={{ fontSize: 11, color: '#6b7280', marginTop: 2 }}>
          Max 15% · Market avg max 7% · Standard 6% · Min 4% · All sources combined
        </div>
      </div>
